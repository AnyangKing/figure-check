...
# ==============================================================================
# 3. 메인 분석부 (모든 모델 통합 및 Figure 6 음악 제외)
# ==============================================================================
if __name__ == '__main__':
    ...
    p_kf_m, p_music_m = np.array(kf_m_list), np.array(music_m_list)

    # Figure 1-3: 평면별 추기도 (MUSIC 포함)
    ...
    for n1, n2, dims, fig_n in planes:
        ...
        for k, p_data in zip(['Proposed', 'MUSIC', 'LSTM', 'MLP', 'KF', 'CNN'], [p_prop_m, p_music_m, p_lstm_m, p_mlp_m, p_kf_m, p_cnn_m]):
            ...
            plt.plot(p_data[:, dims[0]], p_data[:, dims[1]], label=f"{k} (RMSE: {rms_val:.4f}m)", color=model_styles[k]['color'], marker=model_styles[k]['marker'], ls=model_styles[k]['ls'], lw=LINE_WIDTH, markevery=15, alpha=0.8)
        plt.title(f'{n1}-{n2} 평면 추정도 (m)'); plt.xlabel(f'{n1} (m)'); plt.ylabel(f'{n2} (m)'); plt.grid(True, ls=':', alpha=0.6); plt.legend(); plt.tight_layout()

    # Figure 4-6: 분석 그래프 (교수님 지적 반영: Figure 6에서 MUSIC 제외)
    ...
    for fn, steps, res_data, xl, tit, m_ev in configs:
        ...
        for k in model_styles.keys():
            # [핵심 수정] DOA 그래프(fn=6)에서만 MUSIC을 스킵함
            if fn == 6 and k == 'MUSIC': continue
            plt.plot(steps, res_data[k], label=('1D-CNN' if k=='CNN' else k), color=model_styles[k]['color'], marker=model_styles[k]['marker'], ls=model_styles[k]['ls'], lw=LINE_WIDTH, markevery=m_ev, markersize=7)
        plt.yscale('log'); plt.ylim(0.1, 100); plt.gca().yaxis.set_major_formatter(ScalarFormatter()); plt.title(tit); plt.xlabel(xl); plt.ylabel("RMSE (m)"); plt.grid(True, which="both", ls=':', alpha=0.5); plt.legend(); plt.tight_layout()

    # Figure 7: 3D 궤적 시각화 (MUSIC 포함)
    ...
    for k, p_data in zip(['Proposed', 'MUSIC', 'LSTM', 'MLP', 'KF', 'CNN'], [p_prop_m, p_music_m, p_lstm_m, p_mlp_m, p_kf_m, p_cnn_m]):
        ...
        ax7.plot(p_data[:, 0], p_data[:, 1], p_data[:, 2], label=f"{k} (3D RMSE: {rms3:.4f}m)", color=model_styles[k]['color'], marker=model_styles[k]['marker'], ls=model_styles[k]['ls'], lw=LINE_WIDTH, markevery=20, alpha=0.7)
